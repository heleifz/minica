...
class ArrayDatasetTest(unittest.TestCase):
    ...
    def test_set_data(self):
        ...
        self.assertTrue((np.array([1,4,7]).reshape((1,3)) == result[0]).all())
        self.assertTrue((np.array([1]) == result[1]).all())
        result = self.data.read()
        self.assertTrue((np.array([2,5,8]).reshape((1,3)) == result[0]).all())
    ...
    def test_batch(self):
        ...
        self.assertTrue((np.array([[1,2],[4,5]]) == result[0]).all())
